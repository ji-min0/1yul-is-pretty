...
=> 최?: CWD가 루트폴더인 상태에서 filter.py의 json파일 open 실행시 profanity.json이 인식이 안되는 문제가 있네요. (main파일 실행시에는 적용이 되는지 확인 해야함.)
...
### to do
- filter.py파일을 comment.py, post.py 파일에서 import 가능한지 확인해야함.
- 루트폴더에 main으로 사용할 파일을 만들고 사용해야하는 다른 파일들은 정리해서 하위폴더에 따로 정리해야함.